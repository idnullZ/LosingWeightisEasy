package idnull.znz.losing_weight_is_easy.data.services
...
import idnull.znz.losing_weight_is_easy.utils.Constants.FASTEST_LOCATION_INTERVAL
import idnull.znz.losing_weight_is_easy.utils.Constants.LOCATION_UPDATE_INTERVAL
import idnull.znz.losing_weight_is_easy.utils.Constants.NOTIFICATION_CHANNEL_ID
import idnull.znz.losing_weight_is_easy.utils.Constants.NOTIFICATION_CHANNEL_NAME
import idnull.znz.losing_weight_is_easy.utils.Constants.NOTIFICATION_ID
import idnull.znz.losing_weight_is_easy.utils.Constants.TIMER_UPDATE_INTERVAL
...
class TrackingServices : LifecycleService() {
    ...
    companion object {
        ...
        val isTracking = MediatorLiveData<Boolean>()
        val pathPoints = MediatorLiveData<Polylines>()
    }
        ...
        timeRunSeconds.observe(this, Observer {
            if (!serviceKill) {
                val notification = curNotificationBuilder
                    .setContentText(UtilsFun.getFormattedStopWatchTime(it * 1000L, false))
    ...
    private fun updateLocationTracking(isTracking: Boolean) {
        if (isTracking) {
            if (UtilsFun.hasLocationPermission(this)) {
